...
(function($){
	var Citys = new Array(); 
	Citys[0] = new Array("北京","东城区|西城区| 崇文区|宣武区|朝阳区|海淀区|丰台区|石景山区|房山区|通州区|顺义区|大兴区|昌平区| 平谷区|怀柔区|门头沟区"); 
	Citys[1] = new Array("上海","黄浦区|卢湾区|徐汇区|长宁区|静安区|普陀区|闸北区|虹口区|杨浦区|宝山区|闵行区|嘉定区|松江区|青浦区|奉贤区|金山区|浦东新区"); 
	Citys[2] = new Array("天津","和平区|河西区|红桥区|汉沽区|大港区|东丽区|西青区|津南区|北辰区|武清区|宝坻区"); 
	Citys[3] = new Array("重庆","渝中区|大渡口区|大渡口区|沙坪坝区|九龙坡区|南岸区|北碚区|綦江区|双桥区|渝北区|巴南区|万州区|涪陵区|黔江区|长寿区|江津区|合川区|永川区|南川区"); 
	...
	$.fn.citys=function(options){
		...
		this.each(function(){
			var str= '<select name="'+options.p_name+'" id="'+options.p_name+'" style="width:'+options.p_width+'px;" >';
				str+= '<option value="">选择省份</option></select>';
				str+= '<select name="'+options.c_name+'" id="'+options.c_name+'" style="width:'+options.c_width+'px;" >';
				str+= '<option value="">选择市区</option></select>';	
			$(this).html(str);
			...
			if(options.p_val!=""){
				...
				$("#"+options.c_name).empty().append("<option value=''>选择市区</option>");

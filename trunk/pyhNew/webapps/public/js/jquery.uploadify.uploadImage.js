		...
		jQuery.extend(jQuery.fn,{
			uploadify:function(options) {
				jQuery(this).each(function(){
					...
					width           : 107, // The width of the flash button
					...
					removeCompleted : true, // Set to true if you want the queue items to be removed when a file is done percentage
				...
				jQuery(this).bind("uploadifySelect", {'action': settings.onSelect, 'queueID': settings.queueID}, function(event, ID, fileObj) {
					if (event.data.action(event, ID, fileObj) !== false) {
						...
						jQuery(queue).html('<li id="' + jQuery(this).attr('id') + ID + '" class="uploadifyQueueItem">'+'\
								<img src="'+path+'/public/images/usr/uploadbg.jpg" alt="" width="300" />\
						</li>');
						//$(queue).append($(node));
						//<img src="${pageContext.request.contextPath}/public/images/me.jpg" alt="" width="300" />
						//jQuery(queue).html('<img src="'+path+'/public/images/accountsetting/head_default.png" alt="" width="300" />');
						/*jQuery(queue).append('<tr id="' + jQuery(this).attr('id') + ID + '" class="uploadifyQueueItem">'+'\
								<td class="fileName">' + fileName + '</td><td>'+byteSize + suffix+'</td>\
								<td class="uploadifyProgress">\
									<div id="' + jQuery(this).attr('id') + ID + 'ProgressBar" class="uploadifyProgressBar"></div><span class="percentage"></span>\
								</td>\
							</tr>');*/
					};
				});
				...
				jQuery(this).bind("uploadifyQueueFull", {'action': settings.onQueueFull}, function(event, queueSizeLimit) {
					if (event.data.action(event, queueSizeLimit) !== false) {
						alert('队列已经满了。 最大一次允许上传 ' + queueSizeLimit + '个图片。');
					}
				});
					...
					jQuery.post(checkScript, postData, function(data) {
						for(var key in data) {
							if (event.data.action(event, data, key) !== false) {
								var replaceFile = confirm("您想替换这个文件 " + data[key] + "？");
								if (!replaceFile) {
				...
				jQuery(this).bind("uploadifyError", {'action': settings.onError}, function(event, ID, fileObj, errorObj) {
					if (event.data.action(event, ID, fileObj, errorObj) !== false) {
						...
						jQuery("#" + jQuery(this).attr('id') + ID).find('.uploadifyProgress').show();
					...
					if (event.data.action(event, ID, fileObj, data) !== false) {
						jQuery("#" + jQuery(this).attr('id') + ID + "ProgressBar").animate({'width': data.percentage + '%'},250,function() {
							if (data.percentage == 100) {
								jQuery(this).closest('.uploadifyProgress');
							}
						});
				...
				jQuery(this).bind("uploadifyComplete", {'action': settings.onComplete}, function(event, ID, fileObj, response, data) {
					if (event.data.action(event, ID, fileObj, unescape(response), data) !== false) {
						//jQuery("#" + jQuery(this).attr('id') + ID).find('.percentage').text('上传完成！');

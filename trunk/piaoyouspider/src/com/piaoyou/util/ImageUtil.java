/*
 * $Header: /cvsroot/mvnforum/myvietnam/src/net/myvietnam/mvncore/util/ImageUtil.java,v 1.35 2008/06/30 06:04:51 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.35 $
 * $Date: 2008/06/30 06:04:51 $
 *
 * ====================================================================
 *
            ...
            if(imgWidth>maxWidth || imgHeight>maxHeight){
            	 if(realRate>targetRate){
            		 if(imgWidth>maxWidth){
	                     ...
	                     imgWidth=maxWidth;//把宽固定1024
	                     imgHeight=(int)(imgHeight*(temp));
            		 }
                 }else{
                	 if(imgHeight>maxHeight){
                		 double temp=(double)maxHeight/(double)imgHeight;
                		 imgHeight=maxHeight;//把宽固定768
                		 imgWidth=(int)(imgWidth*(temp));
                	 }
                 }
            }
            System.out.println("afterwidth:"+imgWidth);
            System.out.println("afterheight:"+imgHeight);
//          imgWidth or imgHeight could be -1, which is considered as an assertion
           ...
           /* if ((imgWidth > maxWidth) || (imgHeight > maxHeight)) {
              ...
              //  imgWidth  = (int)(imgWidth  * scaleX); imgWidth = (imgWidth == 0) ? 1 : imgWidth;
              //  imgHeight = (int)(imgHeight * scaleY); imgHeight = (imgHeight == 0) ? 1 : imgHeight;
                // scale as needed
             ...
             * 	if(imgWidth > maxWidth && imgHeight <= maxHeight  ){
            		  double scaleX = (double)maxWidth/imgWidth;
                      double scaleY = (double)maxHeight/imgHeight;
                      scaleRatio= (scaleX < scaleY) ? scaleX : scaleY;
                      imgWidth  = (int)(imgWidth  * scaleX); imgWidth = (imgWidth == 0) ? 1 : imgWidth;
                      imgHeight = (int)(imgHeight * scaleY); imgHeight = (imgHeight == 0) ? 1 : imgHeight;
    ...
    if(TrueHeight/TrueWidth>1.2){
       var Multiple = TrueHeight / RePicHeight;
       ThisPic.width = TrueWidth / Multiple;
       ThisPic.height = RePicHeight;
    }
    else{
...
//			读取超时
    ...
    public static void main(String[] args) {
    	compressImg("http://hiphotos.baidu.com/晓紫英/pic/item/4b2b3d4033926e579313c6da.jpg","C://Users//hp//Pictures//a.jpg",100,100);
//        try {
